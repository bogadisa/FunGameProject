...
public class Player{
    public static GameObject generate() {
        SpriteSheet playerSprites = AssetPool.getSpriteSheet("entities/player_3_3_8.png");
        GameObject player = SpriteObject.generate(playerSprites.getSprite(0), 256, 256);
        player.setName("Player");

        AnimationState runDown = new AnimationState();
        runDown.title = "Run Down";
        float defaultFrameTime = 10.2f;
        ...
        runUp.title = "Run Up";
        runUp.addFrame(playerSprites.getSprite(2), defaultFrameTime);
        runUp.addFrame(playerSprites.getSprite(3), defaultFrameTime);
        runUp.setLoop(true);

        
        AnimationState runSideways = new AnimationState();
        runSideways.title = "Run Sideways";
        runSideways.addFrame(playerSprites.getSprite(6), defaultFrameTime);
        runSideways.addFrame(playerSprites.getSprite(7), defaultFrameTime);
