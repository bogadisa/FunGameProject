...
public class TileUtil {
    private Pattern pattern = Pattern.compile("(?<key>(?<A>C|S|P)-(?<Y>[0-9]+))-(?<var>[0-9]+)");
    private Pattern extendedPattern = Pattern
            .compile("(?<A>C|S|P)(?<B>N|E|W|S)(?<C>S|A)(?<D>O|[0-9])(?<X>[0-9]{2})-(?<Y>[0-9]*)-(?<Z>[0-9]*)");

    protected HashMap<String, Tile> tiles;

    public TileUtil(HashMap<String, Tile> tiles) {
        this.tiles = tiles;
    }

    public Matcher readMapMD(String metadata) {
     ...
     * Where:
     * A = {C, S, P}
     * signaling if tile has collision enabled (C), or not (P), or only slows you
     * down (S)
     ...
     * Its purpose is to easily communicate with neighboring tiles to determine
     * their variation
    ...
    Matcher readTileMD(String metadata) {
        Matcher matcher = extendedPattern.matcher(metadata);
        matcher.find();
        return matcher;
    }
